...
if [[ "${1:-}" == "--reset" ]]; then
  ...
  # Two categories of reset targets:
  #   TRACKED_FILES — ship in the repo; rewound via git checkout
  #   UNTRACKED_FILES — added by setup-stack.sh; deleted outright
  TRACKED_FILES=(
  ...
  # src/ is populated by setup-stack.sh from the template — blow it away if the
  # marker shows it was generated (never touch a user-authored src/).
  if [ -d "src" ] && [ ! -f ".ralph-onboarding-kept-src" ]; then
    echo "  Removing: src/ (generated by setup-stack.sh)"
    rm -rf src
  fi
...
# Only surface on the first run — resuming users already answered this.
_REMOTE_URL=$(git remote get-url origin 2>/dev/null || echo "")
if [ ! -f ralph-config.json ] && [ ! -f .onboard-answers.tmp ] && \
   echo "$_REMOTE_URL" | grep -qiE "(jaeyunha|namuh-eng)/ralph-to-ralph"; then
